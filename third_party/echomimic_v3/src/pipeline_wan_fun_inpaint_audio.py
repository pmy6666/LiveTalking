...
class WanFunInpaintAudioPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.mask_processor = VaeImageProcessor(
            vae_scale_factor=self.vae.config.spacial_compression_ratio, do_normalize=False, do_binarize=True, do_convert_grayscale=True
        )
    ...
    def __call__(
        ...
        if not use_longvideo_cfg:
            ...
            partial_latents_length = int((partial_video_length-1) // self.vae.config.temporal_compression_ratio +1)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    while last_frame <= final_latents_length:
                        ...
                        target_shape = (self.vae.config.latent_channels, cur_latent_t, width // self.vae.config.spacial_compression_ratio, height // self.vae.config.spacial_compression_ratio)
                        seq_len = math.ceil((target_shape[2] * target_shape[3]) / (self.transformer.config.patch_size[1] * self.transformer.config.patch_size[2]) * target_shape[1])
